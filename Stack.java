...
public class Stack {
    public static void main(String[] args) {
        ...
        Executor executor = new Executor(Mode.Debug);
        // REPL Execution
        while (true) {
            String code = new String();
            while (true) {
                String enter = this.input("> ");
                code += enter + "\n";
                if (enter == "") {
                    break;
                }
        ...
        switch (type) {
            case STRING:
                ...
                for (int i = 0; i < str.length(); i++) {
                    result.add(new Type(str.substring(i, i + 1)));
                }
                return result;
            case NUMBER:
                return Collections.singletonList(new Type((double) value));
            case BOOL:
                return Collections.singletonList(new Type((boolean) value));
            case LIST:
                return (List<Type>) value;
            case ERROR:
                return Collections.singletonList(new Type("error:" + value));
            case OBJECT:
    ...
    public void evaluateProgram(String code) { // evaluate string as program
        ...
        for (String token : syntax) {
            ...
            try {
                ...
                this.stack.add(new Type(num)); // Push number value on the stack
            } catch (NumberFormatException ignored) {
                if (token.equals("true") || token.equals("false")) {
                    this.stack.add(new Type(Boolean.parseBoolean(token))); // Push bool value on the stack
                } else if (chars[0] == '(' && chars[chars.length - 1] == ')') {
                    // Processing string escape
                    String string = token.substring(1, token.length() - 1);
                    this.stack.add(new Type(string)); // Push string value on the stack
                } else if (chars[0] == '[' && chars[chars.length - 1] == ']') {
                    ...
                    this.stack.add(new Type(token.substring("error:".length()))); // Push error value on the stack
